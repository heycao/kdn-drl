...
class Sample:
    ...
    def calculate_mlu(self, routing_matrix=None):
        """
        Calculate Maximum Link Utilization (MLU) for this sample.
        
        Parameters
        ----------
        routing_matrix : NxN matrix, optional
            Custom routing matrix. If None, uses the sample's routing_matrix.
        
        Returns
        -------
        float
            Maximum Link Utilization (MLU) value.
        """
        ...
        link_capacities = {(u, v): float(G[u][v][0]['bandwidth']) for u, v in G.edges()}
        ...
        for src in range(net_size):
            for dst in range(net_size):
                ...
                bw = traffic['Flows']['0']['AvgBw'] if ('Flows' in traffic and '0' in traffic['Flows']) else traffic.get('AggInfo', {}).get('AvgBw', 0.0)
        ...
        utilizations = [load / cap for (u, v), load in link_loads.items() if (cap := link_capacities[(u, v)]) > 0]
